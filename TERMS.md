# CourierMap Terms of Service

**Effective date:** 26 May 2026
**Version:** 2 (beta)

These Terms govern your use of the CourierMap mobile application. By tapping "I Agree", or by using the app, you agree to be bound by these Terms.

## Beta software

CourierMap is in beta. The app may contain bugs, change without notice, or have brief periods of unavailability. Data you create may be lost in rare cases (we will do our best to avoid this).

## Eligibility

- You are at least 16 years old.
- You are using the app for its intended purpose: a personal aid for rideshare and delivery work.
- Your account information is accurate. You will not impersonate another driver.

## Acceptable use

- Do not post unlawful, harassing, hateful, or deceptive content in ratings, comments, pickup guides, place suggestions, or bug reports.
- Do not attempt to scrape, reverse-engineer, or overload our backend.
- Do not upload anyone else's driver profile screenshot or attempt to claim a verified-driver status that is not yours. Impostor claims can lead to revocation and account suspension.
- Respect the terms of service of any third-party platform you reference (Uber, DoorDash, etc).

## User content

You retain ownership of the content you create in the app. You grant us a worldwide, non-exclusive licence to host, store, and display that content within CourierMap to other users for the purpose of operating the service. You can revoke this licence at any time by deleting the content.

## Driver verification

Verification is best-effort and relies on automated text recognition and AI to read the driver name from screenshots you upload. This automated detection may be imperfect. We may approve, reject, or revoke a verification claim at our discretion. Verification status is not a guarantee that any other user is who they say they are.

## No warranty

The app is provided "as is", without warranties of any kind, express or implied, including merchantability, fitness for a particular purpose, and non-infringement. Map data, place data, and user contributions may be inaccurate or out-of-date — always use your own judgment when driving.

## Limitation of liability

To the maximum extent permitted by law, CourierMap and its operators are not liable for any indirect, incidental, special, consequential, or punitive damages arising out of your use of the app, including lost profits, lost data, or vehicle or personal injury sustained while using the app.

## Termination

You can stop using the app at any time. We can suspend or terminate your account if you breach these Terms or use the app in a way that endangers other users or the service. We will give reasonable notice where practical.

## Governing law

These Terms are governed by the laws of New South Wales, Australia. Disputes will be heard in the courts of New South Wales unless a mandatory consumer-protection law in your jurisdiction provides otherwise.

## Changes to these Terms

We will bump the Terms version and re-prompt you on next launch when we make a material change. Continued use after a new version is accepted constitutes acceptance.

## Contact

**couriermap2026@gmail.com**
